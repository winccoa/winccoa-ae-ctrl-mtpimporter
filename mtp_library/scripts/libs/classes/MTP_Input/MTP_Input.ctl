...
/**
 * @class MTP_Input
 * @brief Represents the MTP_Input class.
 */
class MTP_Input
{
  ...
  private shared_ptr<MTP_Wqc> _qualityCode; //!< The quality code associated with the input.

  /**
   * @brief Constructor for MTP_Input.
   *
   ...
   */
  public MTP_Input(const string &dpeEnabled, const string &dpeValue, const string &dpeInverted, const string &dpeText, const string &dpeQualityCode)
  {
    ...
    _qualityCode = new MTP_Wqc(dpeQualityCode);
...
#event invertedChanged(const bool &inverted) //!< Event triggered when the inverted state changes.
   ...
   */
  public shared_ptr<MTP_Wqc> getQualityCode()
  {
    return _qualityCode;
  }
