...
/**
 * @class MTP_AnaProcessValueIn
 * @brief Represents the MTP_AnaProcessValueIn class.
 */
class MTP_AnaProcessValueIn : MTP_InputElement
{
  private shared_ptr<MTP_OsLevel> _osLevel; //!< The operational state level of the monitored value.
  private float _value; //!< Input of the current value.
  private float _scaleMinCur; //!< Current scale minimum information.
  private float _scaleMaxCur; //!< Current scale maximum information.
  private shared_ptr<MTP_Unit> _unitCur; //!< Current unit information.

  /**
   * @brief Constructor for the MTP_AnaProcessValueIn object.
   *
   * @param dp The data point of the analog process value input.
   */
  public MTP_AnaProcessValueIn(const string &dp) : MTP_InputElement(dp)
  {
    if (!dpExists(getDp() + ".V"))
    {
  ...
  #event valueChanged(const float &value) //!< Event triggered when the current value changes.
