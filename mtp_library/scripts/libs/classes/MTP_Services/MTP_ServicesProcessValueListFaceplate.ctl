...
/**
 * @class MTP_ServicesProcessValueListFaceplate
 * @brief Dynamically renders ProcessValueIn/Out rows for one ServiceControl tab.
 */
class MTP_ServicesProcessValueListFaceplate : MTP_RefBase
{
  private shared_ptr<MTP_Services> _viewModel; //!< Services view model.
  ...
  /**
   * @brief Constructor for MTP_ServicesProcessValueListFaceplate.
   *
   * @param viewModel Services view model.
   * @param direction Row direction ("In" or "Out").
   * @param shapes Shape mapping.
   */
  public MTP_ServicesProcessValueListFaceplate(shared_ptr<MTP_Services> viewModel, const string &direction, const mapping &shapes) : MTP_RefBase(shapes)
  {
  ...
  private string resolveReferencePanel(const string &dp)
  {
    ...
    DebugN("Unsupported process value type in service list:", typeName, dp);
    return "";
  }
};
