...
/**
 * @class ServParamBase
 * @brief Represents the ServParamBase class.
 */
class ServParamBase
{
  ...
  private bool _applyOperator; //!< Indicates if the operator apply functionality is active.
  ...
  private shared_ptr<MTP_Unit> _valueUnit; //!< The unit associated with the monitored value.

  private shared_ptr<MTP_Wqc> _wqc; //!< The quality code associated with the service parameter.
  private shared_ptr<MTP_OsLevel> _osLevel; //!< The operational state level of the service parameter.
  ...
  protected ServParamBase(const string &dp)
  {
    ...
    if (!dpExists(_dp + ".ApplyOp"))
    {
      throw (makeError("", PRIO_SEVERE, ERR_PARAM, (int)ErrCode::DPNOTEXISTENT, _dp + ".ApplyOp"));
    }
    ...
    dpConnect(this, setApplyOperatorCB, _dp + ".ApplyOp");
    ...
    _wqc = new MTP_Wqc(_dp + ".WQC");
    _osLevel = new MTP_OsLevel(_dp + ".OSLevel");
    _valueUnit = new MTP_Unit(_dp + ".VUnit");
  }
  ...
  #event applyOperatorChanged(const bool &applyOperator) //!< Event triggered when the applyOperator state changes.
  ...
  /**
   * @brief Retrieves the operator apply state.
   * @return The operator apply state as a boolean.
   */
  public bool getApplyOperator()
  {
  ...
  */
  ...
  /**
   * @brief Sets the operator apply state.
   *
   * @param applyOperator The new operator apply state.
   */
  public void setApplyOperator(const bool &applyOperator)
  {
    _applyOperator = applyOperator;
    dpSet(_dp + ".ApplyOp", _applyOperator);
  }
  ...
  /**
   * @brief Callback function to set the operator apply state.
   *
   * @param dpe The data point element.
   * @param applyOperator The new operator apply state.
   */
  private void setApplyOperatorCB(const string &dpe, const bool &applyOperator)
  {
    _applyOperator = applyOperator;
    applyOperatorChanged(_applyOperator);
  }
