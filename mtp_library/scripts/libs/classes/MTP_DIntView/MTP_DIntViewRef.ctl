...
/**
 * @class MTP_DIntViewRef
 * @brief Represents the MTP_DIntViewRef class.
 */
class MTP_DIntViewRef : MTP_ViewRef
{
  protected shape _txtValue; //!< Reference to the value text shape.
  protected shape _txtUnit; //!< Reference to the unit text shape.

  /**
   * @brief Constructor for the MTP_DIntViewRef object.
   *
   * @param viewModel The view model instance.
   * @param shapes The mapping of shapes used by the MTP_DIntViewRef object.
   */
  public MTP_DIntViewRef(shared_ptr<MTP_DIntView> viewModel, const mapping &shapes) : MTP_ViewRef(viewModel, shapes)
  {
    classConnect(this, setValueCB, MTP_ViewRef::getViewModel(), MTP_DIntView::valueChanged);
