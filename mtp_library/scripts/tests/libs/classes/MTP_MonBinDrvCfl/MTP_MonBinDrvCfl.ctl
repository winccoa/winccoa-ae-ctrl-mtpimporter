// $License: NOLICENSE
/** Tests for the library: scripts/libs/classes/MTP_MonBinDrvCfl/MTP_MonBinDrvCfl.ctl.

  @file $relPath
  @test Unit tests for the library: scripts/libs/classes/MTP_MonBinDrvCfl/MTP_MonBinDrvCfl.ctl
  @copyright $copyright
  @author d.schermann
 */
...
//-----------------------------------------------------------------------------
// Libraries used (#uses)
#uses "classes/MTP_MonBinDrvCfl/MTP_MonBinDrvCfl" // tested object
#uses "classes/oaTest/OaTest" // oaTest basic class

//-----------------------------------------------------------------------------
/** Tests for MTP_MonBinDrvCfl.ctl
*/
class TstMTP_MonBinDrvCfl : OaTest
{
  private const string _Dpt = "MTP_MonBinDrvCfl";
  private const string _DpExists = "ExistingTestDatapoint";
  private const string _DptInvalidMissingEnabled = "MTP_MonBinDrvCflInvalid1";
  ...
  {
    shared_ptr<MTP_MonBinDrvCfl> monBinDrvCfl = new MTP_MonBinDrvCfl(_DpExists);
    assertFalse(monBinDrvCfl.getEnabled());
    dpSetWait(_DpExists + ".enabled", TRUE);
    delay(0, 200);
    assertTrue(monBinDrvCfl.getEnabled());
    return 0;
  }
  ...
  public int testConstructor_MissingDpes()
  {
    try
    {
      shared_ptr<MTP_MonBinDrvCfl> monBinDrvCfl = new MTP_MonBinDrvCfl(_DpExistsInvalidMissingEnabled);
  ...
  {
    shared_ptr<MTP_MonBinDrvCfl> monBinDrvCfl = new MTP_MonBinDrvCfl(_DpExists);
    classConnect(this, enabledChangedCB, monBinDrvCfl, MTP_MonBinDrvCfl::enabledChanged);
    _eventEnabled = false;
    dpSetWait(_DpExists + ".enabled", TRUE);
    delay(0, 200);
