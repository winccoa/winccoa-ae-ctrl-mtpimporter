// $License: NOLICENSE
/** Tests for the library: scripts/libs/classes/MTP_OsLevel/MTP_OsLevel.ctl.

  @file $relPath
  @test Unit tests for the library: scripts/libs/classes/MTP_OsLevel/MTP_OsLevel.ctl
  @copyright $copyright
  @author m.woegrath
*/
...
// Libraries used (#uses)
#uses "std"
#uses "classes/MTP_OsLevel/MTP_OsLevel" // tested object
...
//-----------------------------------------------------------------------------
/** Tests for MTP_OsLevel.ctl
*/
class TstMTP_OsLevel : OaTest
{
  ...
  public int setUp() override
  {
    ...
    if (dpTypes(_Dpt).count() == 0)
      result = dpTypeCreate(dpes, values);
  ...
  /**
    @test Verifies that the MTP_OsLevel constructor initializes correctly with a valid data point.
  */
  public int testConstructor()
  {
    shared_ptr<MTP_OsLevel> osLevel = new MTP_OsLevel(_DpExists + ".osLevel");
  ...
  /**
    @test Verifies that the MTP_OsLevel constructor throws an error for a non-existing data point.
  */
  public int testConstructor_NonExistingDatapoint()
  {
    try
    {
      shared_ptr<MTP_OsLevel> osLevel = new MTP_OsLevel("NonExisting.Dpe");
  ...
  */
  ...
  {
    shared_ptr<MTP_OsLevel> osLevel = new MTP_OsLevel(_DpExists + ".osLevel");
    classConnect(this, setOsStationLevelCB, osLevel, MTP_OsLevel::osStationLevelChanged);
