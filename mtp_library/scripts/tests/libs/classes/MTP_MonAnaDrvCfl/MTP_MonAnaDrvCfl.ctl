// $License: NOLICENSE
/** Tests for the library: scripts/libs/classes/MTP_MonAnaDrvCfl/MTP_MonAnaDrvCfl.ctl.

  @file $relPath
  @test Unit tests for the library: scripts/libs/classes/MTP_MonAnaDrvCfl/MTP_MonAnaDrvCfl.ctl
  @copyright $copyright
  @author d.schermann
 */
...
class TstMTP_MonAnaDrvCfl : OaTest
{
  private const string _Dpt = "MTP_MonAnaDrvCfl";
  private const string _DpExists = "ExistingTestDatapoint";
  private const string _DptInvalidMissingEnabled = "MTP_MonAnaDrvCflInvalid1";
  ...
  {
    shared_ptr<MTP_MonAnaDrvCfl> monAnaDrvCfl = new MTP_MonAnaDrvCfl(_DpExists);
    assertFalse(monAnaDrvCfl.getEnabled());
    dpSetWait(_DpExists + ".enabled", TRUE);
    delay(0, 200);
    assertTrue(monAnaDrvCfl.getEnabled());
    return 0;
  }
  ...
  public int testConstructor_MissingDpes()
  {
    try
    {
      shared_ptr<MTP_MonAnaDrvCfl> monAnaDrvCfl = new MTP_MonAnaDrvCfl(_DpExistsInvalidMissingEnabled);
  ...
  {
    shared_ptr<MTP_MonAnaDrvCfl> monAnaDrvCfl = new MTP_MonAnaDrvCfl(_DpExists);
    classConnect(this, enabledChangedCB, monAnaDrvCfl, MTP_MonAnaDrvCfl::enabledChanged);
    _eventEnabled = false;
    dpSetWait(_DpExists + ".enabled", TRUE);
    delay(0, 200);
