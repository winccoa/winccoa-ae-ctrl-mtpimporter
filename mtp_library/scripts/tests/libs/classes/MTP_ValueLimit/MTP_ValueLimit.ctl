...
/** Tests for the library: scripts/libs/classes/MtpValueLimit/MtpValueLimit.ctl.
  ...
  @file $relPath
  @test Unit tests for the library: scripts/libs/classes/MTP_ValueLimit/MTP_ValueLimit.ctl
  @copyright $copyright
  @author m.woegrath
 */
...
// Libraries used (#uses)
#uses "std"
#uses "classes/MTP_ValueLimit/MTP_ValueLimit"// tested object
#uses "classes/oaTest/OaTest" // oaTest basic class

//-----------------------------------------------------------------------------
// Variables and Constants
class MTP_ValueLimitTest : MTP_ValueLimit
{
  public MTP_ValueLimitTest(const string &dpeLimit, const string &dpeEnabled, const string &dpeActive): MTP_ValueLimit(dpeLimit, dpeEnabled, dpeActive)
  {
...
class TstMtpValueLimit : OaTest
{
  ...
  public int testConstructor_Protected()
  {
    try
    {
      shared_ptr<MTP_ValueLimit> limit = new MTP_ValueLimit(_DpExists + ".limit", _DpExists + ".enabled", _DpExists + ".active");
    }
    catch
    {
  ...
  {
    shared_ptr<MTP_ValueLimit> limit = new MTP_ValueLimitTest(_DpExists + ".limit", _DpExists + ".enabled", _DpExists + ".active");
  ...
  public int testConstructor_NonExistingDpes()
  {
    try
    {
      shared_ptr<MTP_ValueLimit> limit = new MTP_ValueLimitTest(_DpExists + ".noneLimit", _DpExists + ".enabled", _DpExists + ".active");
    ...
    try
    {
      shared_ptr<MTP_ValueLimit> limit = new MTP_ValueLimitTest(_DpExists + ".limit", _DpExists + ".noneEnabled", _DpExists + ".active");
      assertTrue(false, "shouldn't reach here");
    }
    catch
    {
    ...
    try
    {
      shared_ptr<MTP_ValueLimit> limit = new MTP_ValueLimitTest(_DpExists + ".limit", _DpExists + ".enabled", _DpExists + ".noneActive");
      assertTrue(false, "shouldn't reach here");
    }
    catch
    {
  ...
  {
    shared_ptr<MTP_ValueLimitTest> limit = new MTP_ValueLimitTest(_DpExists + ".limit", _DpExists + ".enabled", _DpExists + ".active");
    classConnect(this, setActiveCB, limit, MTP_ValueLimitTest::activeChanged);
