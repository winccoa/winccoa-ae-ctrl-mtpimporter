// $License: NOLICENSE
/** Tests for the library: scripts/libs/classes/MTP_MonBinVlvCfl/MTP_MonBinVlvCfl.ctl.

  @file $relPath
  @test Unit tests for the library: scripts/libs/classes/MTP_MonBinVlvCfl/MTP_MonBinVlvCfl.ctl
  @copyright $copyright
  @author d.schermann
 */
...
//-----------------------------------------------------------------------------
// Libraries used (#uses)
#uses "classes/MTP_MonBinVlvCfl/MTP_MonBinVlvCfl" // tested object
#uses "classes/oaTest/OaTest" // oaTest basic class

//-----------------------------------------------------------------------------
/** Tests for MTP_MonBinVlvCfl.ctl
*/
class TstMTP_MonBinVlvCfl : OaTest
{
  private const string _Dpt = "MTP_MonBinVlvCfl";
  private const string _DpExists = "ExistingTestDatapoint";
  private const string _DptInvalidMissingEnabled = "MTP_MonBinVlvCflInvalid1";
  ...
  {
    shared_ptr<MTP_MonBinVlvCfl> monBinVlvCfl = new MTP_MonBinVlvCfl(_DpExists);
    assertFalse(monBinVlvCfl.getEnabled());
    dpSetWait(_DpExists + ".enabled", TRUE);
    delay(0, 200);
    assertTrue(monBinVlvCfl.getEnabled());
    return 0;
  }
  ...
  public int testConstructor_MissingDpes()
  {
    try
    {
      shared_ptr<MTP_MonBinVlvCfl> monBinVlvCfl = new MTP_MonBinVlvCfl(_DpExistsInvalidMissingEnabled);
  ...
  {
    shared_ptr<MTP_MonBinVlvCfl> monBinVlvCfl = new MTP_MonBinVlvCfl(_DpExists);
    classConnect(this, enabledChangedCB, monBinVlvCfl, MTP_MonBinVlvCfl::enabledChanged);
    _eventEnabled = false;
    dpSetWait(_DpExists + ".enabled", TRUE);
    delay(0, 200);
