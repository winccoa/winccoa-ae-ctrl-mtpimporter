...
//   execScript("#uses \"classes/GUI/faceplate\"\nmain(){}");
    ...
    DebugN("ggggggggggg", g_openFaceplateTriggerEvent);
...
void hook_faceplateTabsToOpen(string sDp, dyn_string &ds_panels, dyn_string &ds_pfad)
{
  ...
  {
    int pos = strpos(ds_panels.at(i), "_");
    string basePanel;
    if (pos > 0)
    {
      basePanel = substr(ds_panels.at(i), 0, pos);
    }
    if (ds_panels.at(i).contains("setting_") || (pos > 0 && (dynContains(ds_panels,basePanel + ".pnl") > 0 || dynContains(ds_panels,basePanel + ".xml") > 0 )))
    {
